...
describe("MCP tool error handling", () => {
  ...
  it("propagates AuthenticationError from SDK", async () => {
    avala.transport.requestPage.mockRejectedValue(new AuthenticationError("Invalid API key"));
  ...
  it("propagates NotFoundError from SDK", async () => {
    avala.transport.requestSingle.mockRejectedValue(new NotFoundError("Dataset not found"));
  ...
  it("propagates generic AvalaError from SDK", async () => {
    avala.transport.requestPage.mockRejectedValue(new AvalaError("Something went wrong", 500));
