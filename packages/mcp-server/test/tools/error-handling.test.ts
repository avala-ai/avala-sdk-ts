...
describe("MCP tool error handling", () => {
  ...
  it("propagates AuthenticationError from SDK", async () => {
    avala.transport.requestPage.mockRejectedValue(
  ...
  it("propagates NotFoundError from SDK", async () => {
    avala.transport.requestSingle.mockRejectedValue(
      new NotFoundError("Dataset not found"),
    );

    const handler = server.getHandler("get_dataset")!;
    await expect(handler({ uid: "nonexistent" })).rejects.toThrow(
      NotFoundError,
    );
  });
  ...
  it("propagates generic AvalaError from SDK", async () => {
    avala.transport.requestPage.mockRejectedValue(
      new AvalaError("Something went wrong", 500),
    );
